...
/* Created on:     25-06-2012 11:46:49                          */
/*==============================================================*/
...
alter table IFRS_RELACION_EEFF
   drop constraint FK_IFRS_REL_REFERENCE_IFRS_COD;
...
create table IFRS_EMPRESA  (
   ...
   GIRO                 VARCHAR(256),
   constraint PK_IFRS_EMPRESA primary key (RUT)
);
...
create table IFRS_GRUPO_EMPRESA  (
   ID_GRUPO_ACCESO      VARCHAR2(128 CHAR)              not null,
   RUT                  NUMBER(4)                       not null,
   constraint PK_IFRS_GRUPO_EMPRESA primary key (ID_GRUPO_ACCESO, RUT)
);
...
comment on table IFRS_PERIODO is
...
create table IFRS_RELACION_DETALLE_EEFF  (
   ...
   MONTO_PESOS          NUMBER(18,4),
   MONTO_MILES          NUMBER(18,4),
   constraint PK_IFRS_RELACION_DETALLE_EEFF primary key (ID_FECU, ID_CUENTA, ID_PERIODO)
);
...
create table IFRS_RELACION_EEFF  (
   ...
   ID_FILA              NUMBER(4,0),
   MONTO_TOTAL          NUMBER(18,4),
   constraint PK_IFRS_RELACION_EEFF primary key (ID_FECU, ID_PERIODO)
);
...
alter table IFRS_GRUPO_EMPRESA
   add constraint FK_IFRS_GRU_REFERENCE_IFRS_EMP foreign key (RUT)
      references IFRS_EMPRESA (RUT);

alter table IFRS_HISTORIAL_REPORTE
   add constraint FK_IFRS_HIS_REFERENCE_IFRS_PER foreign key (ID_PERIODO)
...
alter table IFRS_RELACION_DETALLE_EEFF
   add constraint FK_IFRS_REL_REFERENCE_IFRS_PER foreign key (ID_PERIODO)
      references IFRS_PERIODO (ID_PERIODO);
...
alter table IFRS_RELACION_EEFF
   add constraint FK_IFRS_REL_REFERENCE_IFRS_PE2 foreign key (ID_PERIODO)
      references IFRS_PERIODO (ID_PERIODO);
...
alter table IFRS_VERSION
   add constraint FK_IFRS_VER_REFERENCE_IFRS_PER foreign key (ID_PERIODO)
      references IFRS_PERIODO (ID_PERIODO);

alter table IFRS_VERSION_EEFF
   add constraint FK_IFRS_VER_REFERENCE_IFRS_PE2 foreign key (ID_PERIODO)
      references IFRS_PERIODO (ID_PERIODO);
