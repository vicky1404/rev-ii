...
public class SAXBuilder {
    ...
    public DiscoverableTaxonomySet build(InputSource source) throws IOException, SAXException {
        topTaxonomy = true;
        ...
        if (baseDir == null) {
            baseDir = URI.create(source.getSystemId().substring(0, source.getSystemId().lastIndexOf("/") + 1));
        }
    ...
    private void parseLinkbases(InputSource source, String role) throws SAXException, IOException {
        /** Label Linkbase */
        ...
        else if (role.equals("http://www.xbrl.org/2003/role/presentationLinkbaseRef")) {
        ...
        /** Definition Linkbase */
        else if (role.equals("http://www.xbrl.org/2003/role/definitionLinkbaseRef")) {
        ...
        else if (role.equals("http://www.xbrl.org/2003/role/calculationLinkbaseRef")) {
    ...
    private DiscoverableTaxonomySet parseSchema(InputSource source) throws SAXException, IOException {
		 ...
		 */
        newSchema.setName(source.getSystemId().substring(source.getSystemId().lastIndexOf("/") + 1,
        ...
        Map importedSchemaFiles = xbrlSchemaContentHandler.getImportedSchemaFiles();
        newSchema.setImportedTaxonomyNames(new HashSet(importedSchemaFiles.values()));
        ...
        while (i.hasNext()) {
            String currSchemaLocation = (String)importedSchemaFiles.get(i.next());

            URI uri = URI.create(currSchemaLocation);

            Iterator fixedSchemaFilesIterator = fixedSchemaFiles.keySet().iterator();
            if (uri.isAbsolute()) {
                while (fixedSchemaFilesIterator.hasNext()) {
                    ...
                    if (uri.toString().equals(key)) {
                        parseSchema(new InputSource("file://tacora/usuarios/Rodrigo.Reyes/xbrl-taxonomia/schemaFiles/" + value));
                    }
                }
            }

            else {
                parseSchema(new InputSource(baseDir.toString() + currSchemaLocation));
            }
