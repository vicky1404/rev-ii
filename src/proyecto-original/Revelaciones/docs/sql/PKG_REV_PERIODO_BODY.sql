                ...
                values (V_ID_ESTRUCTURA, C_GRILLA_ROW.TITULO, C_GRILLA_ROW.TIPO_FORMULA);
                  ...
                  for C_COLUMNA_ROW in (select * from REV_COLUMNA where ID_GRILLA = C_ESTRUCTURA_ROW.ID_ESTRUCTURA ) LOOP
                        ...
                        for C_CELDA_ROW in (select * from REV_CELDA where ID_GRILLA = C_ESTRUCTURA_ROW.ID_ESTRUCTURA and ID_COLUMNA = C_COLUMNA_ROW.ID_COLUMNA) LOOP
                              ...
                              insert into REV_CELDA (ID_COLUMNA, ID_FILA, ID_TIPO_CELDA, ID_TIPO_DATO, VALOR, GRUPO, ID_GRILLA, GRUPO_RESULTADO) 
                              values (C_COLUMNA_ROW.ID_COLUMNA, C_CELDA_ROW.ID_FILA, C_CELDA_ROW.ID_TIPO_CELDA, C_CELDA_ROW.ID_TIPO_DATO, C_CELDA_ROW.VALOR, C_CELDA_ROW.GRUPO, V_ID_ESTRUCTURA, C_CELDA_ROW.GRUPO_RESULTADO); 
                              
                        end LOOP;
                        
                  end LOOP;
                  
            FOR C_CAMPO_FORMULA_ROW in (select * from REV_CAMPO_FORMULA where ID_GRILLA = C_ESTRUCTURA_ROW.ID_ESTRUCTURA) LOOP --AKI
                                          
                V_ID_CAMPO_FORMULA := SEQ_CAMPO_FORMULA.NEXTVAL;
                
                --DBMS_OUTPUT.PUT_LINE('V_ID_CAMPO_FORMULA ' || V_ID_CAMPO_FORMULA || ' C_CAMPO_FORMULA_ROW.ID_COLUMNA ' || C_CAMPO_FORMULA_ROW.ID_COLUMNA || ' C_CAMPO_FORMULA_ROW.ID_FILA ' || C_CAMPO_FORMULA_ROW.ID_FILA || ' V_ID_ESTRUCTURA ' || V_ID_ESTRUCTURA);
                ...
                --DBMS_OUTPUT.PUT_LINE('PASANDO 18');
  
                FOR C_FORMULA_GRILLA_ROW IN (SELECT * FROM REV_FORMULA_GRILLA WHERE ID_CAMPO_FORMULA = C_CAMPO_FORMULA_ROW.ID_CAMPO_FORMULA) LOOP --AKI
                
                     --DBMS_OUTPUT.PUT_LINE('ID_TIPO_OPERACION '|| C_FORMULA_GRILLA_ROW.ID_TIPO_OPERACION ||  ' V_ID_CAMPO_FORMULA ' ||  V_ID_CAMPO_FORMULA || ' ID_COLUMNA ' || C_FORMULA_GRILLA_ROW.ID_COLUMNA ||  ' ID_FILA ' || C_FORMULA_GRILLA_ROW.ID_FILA || ' V_ID_ESTRUCTURA ' || V_ID_ESTRUCTURA);
            
                     insert into REV_FORMULA_GRILLA (ID_FORMULA, ID_TIPO_OPERACION, ID_CAMPO_FORMULA, ID_COLUMNA, ID_FILA, ID_GRILLA)
                     values (SEQ_FORMULA_GRILLA.NEXTVAL, C_FORMULA_GRILLA_ROW.ID_TIPO_OPERACION, V_ID_CAMPO_FORMULA, C_FORMULA_GRILLA_ROW.ID_COLUMNA, C_FORMULA_GRILLA_ROW.ID_FILA, V_ID_ESTRUCTURA);
                     
                     --DBMS_OUTPUT.PUT_LINE('PASANDO 19');
