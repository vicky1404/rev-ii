...
public class ComponenteBackingBean extends SoporteBackingBean implements Serializable {
    ...
    private List<GrupoEeff> grupoEeffList;
    ...
    public List<SelectItem> getCatalogoSelectItem(){
        ...
        try {
            catalogoList = this.getFacade().getCatalogoService().findCatalogoByFiltro(super.getNombreUsuario(), null, null, VigenciaEnum.VIGENTE.getKey());
            
            //for(Catalogo catalogo : getCatalogoList()){ //Se comenta por servicio perfilación.
    ...
    public List<GrupoEeff> getGrupoEeffList() throws Exception {
     ...
     * @return
     */
    public List<SelectItem> getGruposEeff(){        
        List<SelectItem> gruposEeff = new ArrayList<SelectItem>();
        try {
            for (GrupoEeff grupoEeff : this.getGrupoEeffList()) {
                if (grupoEeff.isVigencia()){
                    gruposEeff.add(new SelectItem(grupoEeff, grupoEeff.getDescripcion()));
                }
            }
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
