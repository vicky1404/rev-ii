...
public class PresentationLinkbase extends Linkbase {
    ...
    public PresentationLinkbase(DiscoverableTaxonomySet dts) {
        ...
        linkRoleToElementList = new TreeMap<String, List<PresentationLinkbaseElement>>();
    }
    ...
    public void buildPresentationLinkbase() {
        ...
        while (extendedLinkRolesIterator.hasNext()) {
            ...
            List<PresentationLinkbaseElement> presentationLinkbaseElementList =
                new ArrayList<PresentationLinkbaseElement>();

            List<ExtendedLinkElement> extendedLinkElementList =
                getExtendedLinkElementsFromBaseSet(currExtendedLinkRole);
            ...
            while (extendedLinkElementIterator.hasNext()) {
                ...
                PresentationLinkbaseElement currPresentationLinkbaseElement =
                    new PresentationLinkbaseElement((Locator)currExtendedLinkElement);
                currPresentationLinkbaseElement.setExtendedLinkRole(currExtendedLinkRole);

                /* set successor elements */
                List<ExtendedLinkElement> xLinkElementsSuccessor =
                    getTargetExtendedLinkElements(currXBRLElement, currExtendedLinkRole);
                ...
                List<ExtendedLinkElement> xLinkList =
                    getSourceExtendedLinkElements(currXBRLElement, currExtendedLinkRole);
                if (xLinkList.size() > 0) {
                ...
                if (xbrlElementSuccessor.size() == 0) {
                    ...
                    int numberOfSuccessorAtDeepestLevel =
                        determineNumberOfSuccessorAtDeepestLevel(0, currXBRLElement, currExtendedLinkRole);
    ...
    public List<PresentationLinkbaseElement> getPresentationList(String taxonomyName,
        ...
        for (int i = 0; i < resultList.size(); i++) {
            ...
            if ((taxonomyName != null && currElement.getConcept().getTaxonomySchemaName().equals(taxonomyName)) ||
                taxonomyName == null) {
    ...
    public PresentationLinkbaseElement getPresentationLinkbaseElement(Concept tmpElement, String extendedLinkRole) {
        ...
        List<PresentationLinkbaseElement> presentationLinkbaseElementList =
            linkRoleToElementList.get(extendedLinkRole);
        if (presentationLinkbaseElementList != null) {
            Iterator<PresentationLinkbaseElement> presentationLinkbaseElementListIterator =
                presentationLinkbaseElementList.iterator();
     ...
     */
    private List<PresentationLinkbaseElement> collectPresentationLinkbaseElementList(PresentationLinkbaseElement currElement,
        ...
        while (successorElementsIterator.hasNext()) {
            ...
            PresentationLinkbaseElement nextElement =
                getPresentationLinkbaseElement(currXBRLElement, extendedLinkRole);
        ...
        if (currXLinkElement.size() == 0) {
            ...
            while (currXLinkElementIterator.hasNext()) {
                ...
                if (xLinkElementChild.isLocator()) {
                    i =
  determineNumberOfSuccessorAtDeepestLevel(i, ((Locator)xLinkElementChild).getConcept(), extendedLinkRole);
                }
            }
        }
        return i;
    ...
    public List<PresentationLinkbaseElement> getPresentationLinkbaseElementRoot(String extendedLinkRole) {
        ...
        if (presentationLinkbaseElementList != null) {
            Iterator<PresentationLinkbaseElement> presentationLinkbaseElementListIterator =
                presentationLinkbaseElementList.iterator();
            while (presentationLinkbaseElementListIterator.hasNext()) {
                PresentationLinkbaseElement currElement =
                    (PresentationLinkbaseElement)presentationLinkbaseElementListIterator.next();
    ...
    public void setLinkRoleToElementList(Map<String, List<PresentationLinkbaseElement>> linkRoleToElementList) {
