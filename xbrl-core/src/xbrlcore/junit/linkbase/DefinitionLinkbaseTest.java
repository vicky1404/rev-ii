...
public final class DefinitionLinkbaseTest {
    ...
    public void hypercube() {
        ...
        Hypercube hcEuropa = stTaxonomy.getHypercube(stTaxonomy.getConceptByID("t-st_hcEuropa"));
        Hypercube hcAsien = stTaxonomy.getHypercube(stTaxonomy.getConceptByID("t-st_hcAsien"));
        Hypercube hcAmerika = stTaxonomy.getHypercube(stTaxonomy.getConceptByID("t-st_hcAmerika"));
		...
		while (hcEuropaDimensionsIterator.hasNext()) {
			...
			assertTrue(currDimElement.getId().equals("t-st_KontinentDimension") || currDimElement.getId().equals("d-la_LaenderDimension"));
			if (currDimElement.getId().equals("t-st_KontinentDimension")) {
				Set<ExtendedLinkElement> dimensionDomain = hcEuropa.getDimensionDomain(currDimElement);
				assertEquals(2, dimensionDomain.size());
				Concept koKontinente = stTaxonomy.getConceptByID("d-ko_Kontinente");
				...
				while (dimensionDomainIterator.hasNext()) {
					ExtendedLinkElement currXLinkElement = (ExtendedLinkElement) dimensionDomainIterator.next();
					Concept currDomainElement = ((Locator) currXLinkElement).getConcept();
					assertTrue(currDomainElement.getId().equals("d-ko_Kontinente") || currDomainElement.getId().equals("d-ko_Europa"));
				}
        ...
        assertTrue(hcEuropa.containsDimension(stTaxonomy.getConceptByID("d-la_LaenderDimension")));

        assertTrue(hcEuropa.containsDimensionDomain(stTaxonomy.getConceptByID("d-la_LaenderDimension"), stTaxonomy.getConceptByID("d-la_Deutschland")));
        assertTrue(hcAsien.containsDimensionDomain(stTaxonomy.getConceptByID("t-st_KontinentDimension"), stTaxonomy.getConceptByID("d-ko_Asien")));
        assertFalse(hcAmerika.containsDimensionDomain(stTaxonomy.getConceptByID("t-st_KontinentDimension"), stTaxonomy.getConceptByID("d-la_Deutschland")));

        assertTrue(hcAsien.containsDimensionDomain(stTaxonomy.getConceptByID("t-st_KontinentDimension"), stTaxonomy.getConceptByID("d-ko_Kontinente")));
        assertFalse(hcAsien.containsUsableDimensionDomain(stTaxonomy.getConceptByID("t-st_KontinentDimension"), stTaxonomy.getConceptByID("d-ko_Kontinente")));
    }
    ...
    public void dimensions() {
         ...
         */
        try {
            Concept dimKontinent = stTaxonomy.getConceptByID("t-st_KontinentDimension");
