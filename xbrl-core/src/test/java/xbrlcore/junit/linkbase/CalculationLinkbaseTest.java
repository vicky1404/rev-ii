...
public class CalculationLinkbaseTest {
    ...
    public static void setUp() {
        try {
            ...
            String filename = PATH  + "instance.xml";
			instance = instanceFactory.createInstance(new File(CalculationLinkbase.class.getResource(filename).toURI()));
        } catch (Exception ex) {
	...
	public void calcLinkbase() {
		try {
			...
			Map<Concept, Float> calcMap = calcLinkbase.getCalculations(
					primItem, "http://www.xbrl.org/2003/role/link");
	...
	public void singleFactToCalculate() {
		try {
			...
			Iterator<DiscoverableTaxonomySet> iterator = instance
					.getDiscoverableTaxonomySet().iterator();
			@SuppressWarnings("unused")
			CalculationLinkbase calcLinkbase = iterator.next()
					.getCalculationLinkbase();
