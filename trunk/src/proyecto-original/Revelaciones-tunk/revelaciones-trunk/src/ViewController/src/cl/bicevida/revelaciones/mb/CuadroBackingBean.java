...
public class CuadroBackingBean extends SoporteBackingBean implements Serializable{
    ...
    public void validarEstructuraListener(ActionEvent event){        
        ...
        for(Estructura estructuras : this.getEstructuraList()){            
            if(estructuras.getTipoEstructura().getIdTipoEstructura().equals(TipoEstructura.ESTRUCTURA_TIPO_GRILLA)){
                try {                    
                    ...
                    result = getFacade().getFormulaService().processValidatorGrillas(estructuras.getGrillaVO().getGrilla());
        ...
        try{            
            ...
            actualizarValidado(valid ? VigenciaEnum.VIGENTE.getKey() : VigenciaEnum.NO_VIGENTE.getKey()); //Actualiza estado validado deacuerdo con la validacion de EEFF.
            actualizarValidado(validCelda ? VigenciaEnum.VIGENTE.getKey() : VigenciaEnum.NO_VIGENTE.getKey());//Actualiza estado validado deacuerdo con la validacion entre grillas.
        ...
        if (!validCelda){
            agregarWarnMessage(PropertyManager.getInstance().getMessage("general_mensaje_validado_mal_grilla"));
        }
    ...
    public void verSubCeldas(){
        
        List<SubCelda> subCeldaList = new ArrayList<SubCelda>();
        
        for (SubGrilla subGrilla : this.getColumna().getGrilla().getSubGrillaList()){
            
            if (subGrilla.getGrupo() != null && subGrilla.getGrupo().getIdGrupo() != null){
                
                    for (SubColumna subColumna : subGrilla.getSubColumnaList()){
                        for (SubCelda subCelda : subColumna.getSubCeldaList()){
                            if (subCelda.getIdSubFila().equals(this.getIdFila()) && subCelda.getIdSubColumna().equals(this.getIdColumna())){
            ...
            this.setSubCeldaList(subCeldaList);
            //displayPopup();
        }
        ...
        private void displayPopup(){
            ...
            extRenderKitSrvc.addScript(context, "AdfPage.PAGE.findComponent('::detalleDesagregacionesPopUp').show();");
        }
