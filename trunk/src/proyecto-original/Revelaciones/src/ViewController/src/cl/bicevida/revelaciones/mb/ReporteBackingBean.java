...
public class ReporteBackingBean extends SoporteBackingBean implements Serializable{
    ...
    public void generarExcelForPrint(ActionEvent actionEvent){        
        try {
            Version version = (Version)actionEvent.getComponent().getAttributes().get("version");
            List<Version> versionList = new ArrayList<Version>();
            versionList.add(version);
            for(Version v : versionList){
                List<Estructura> estructuras = getFacade().getEstructuraService().getEstructuraByVersion(v, Boolean.FALSE);
                version.setEstructuraList(estructuras);
            }            
            List<ReportePrincipalVO> reportes = getReporteUtilBackingBean().getGenerarListReporteVO(versionList);                            
