 ...
 * http://www.xbrl.org/SpecRecommendations/. <br/>
 * <br/>
 * 
 * @author Daniel Hamm
 */
...
public class InstanceContext implements Serializable {
	...
	private List<Element> scenarioElements; // List of elements within the
											// scenario element (besides
											// dimensional information)

	private List<Element> segmentElements; // List of elements within the
											// segment element (besides
											// dimensional information

	private MultipleDimensionType mdtScenario; // represents the dimensions and
												// domains this context refers
												// to in the <scenario> element

	private MultipleDimensionType mdtSegment; // represents the dimensions and
												// domains this context refers
	 ...
	 * if and only if: <br/>
	 * - both have the same ID <br/>
	 * - both refer to the same period <br/>
	 * - both have the same identifier and identifier scheme <br/>
	 * - the multidimensional information in each part of the context (
	 * <segment>or <scenario>) of both objects is equal <br/>
	 * - both have the same elements in their <segment>and <scenario>part
	 * 
	...
	public boolean equals(Object obj) {
		...
		return id.equals(otherContext.getId())
				&& (identifierScheme == null ? otherContext.getIdentifierScheme() == null : identifierScheme.equals(otherContext.getIdentifierScheme()))
				&& (identifier == null ? otherContext.getIdentifier() == null : identifier.equals(otherContext.getIdentifier()))
				&& (periodValue == null ? otherContext.getPeriodValue() == null : periodValue.equals(otherContext.getPeriodValue()))
				&& scenarioElements.equals(otherContext.getScenarioElements())
				&& segmentElements.equals(otherContext.getSegmentElements())
				&& (mdtScenario == null ? otherContext.getDimensionalInformation(GeneralConstants.DIM_SCENARIO) == null : mdtScenario.equals(otherContext
						.getDimensionalInformation(GeneralConstants.DIM_SCENARIO)))
				&& (mdtSegment == null ? otherContext.getDimensionalInformation(GeneralConstants.DIM_SEGMENT) == null : mdtSegment.equals(otherContext
	...
	public int hashCode() {
		...
		hash = hash * 31 + (identifierScheme != null ? identifierScheme.hashCode() : 0);
	 ...
	 */
	public void setDimensionalInformation(MultipleDimensionType mdt, int scen_seg) throws InstanceException {
		try {
			if (mdt == null)
				return;
			if (scen_seg == GeneralConstants.DIM_SCENARIO)
