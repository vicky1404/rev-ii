...
public final class XBRLInstanceTest {
	...
	public static void setUp() {
		try {
			...
			// + "t-cs-2005-12-31.xsd"));
			opTemplateTaxonomy = TestHelper.getDTS("xbrl/taxonomies1.0/t-op-2005-12-31.xsd");

			opPrimaryTaxonomy = opTemplateTaxonomy.getTaxonomySchema("p-op-2005-12-31.xsd");
			baDimensionTaxonomy = opTemplateTaxonomy.getTaxonomySchema("d-ba-2005-12-31.xsd");
			blDimensionTaxonomy = opTemplateTaxonomy.getTaxonomySchema("d-bl-2005-12-31.xsd");
			...
			// File(PATH
			// + "t-mc-2005-12-31.xsd"));
			mkrSaComTemplateTaxonomy = TestHelper.getDTS("xbrl/taxonomies1.0/t-mc-2005-12-31.xsd");
			assertNotNull(mkrSaComTemplateTaxonomy);

			/* Creation of own MultipleDimensionType for testing purposes */
			Concept dimensionalElementBL = opTemplateTaxonomy.getConceptByID("t-op_BusinessLinesDimension");
			Concept domainElementBL = opTemplateTaxonomy.getConceptByID("d-bl_CorporateFinance");
			Concept dimensionalElementBA = opTemplateTaxonomy.getConceptByID("t-op_BankingActivitiesDimension");
			Concept domainElementBA = opTemplateTaxonomy.getConceptByID("d-ba_TotalBankingActivitiesSubjectSTA");
			...
			myMDT = new MultipleDimensionType(dimensionalElementBL, domainElementBL);
			myMDT.addPredecessorDimensionDomain(new SingleDimensionType(dimensionalElementBA, domainElementBA));
			...
			capitalRequirements = opTemplateTaxonomy.getConceptByID("p-op_CapitalRequirements");
			assertNotNull(year3);
			assertNotNull(year2);
			...
			year2Fact.setPrecision(Integer.MAX_VALUE);
			...
			capitalRequirementsFact.setInstanceUnit(InstanceUnitFactory.getUnit4217EUR());

			/* Creation of MultipleDimensionType objects */
			MultipleDimensionType mdt = new MultipleDimensionType(opTemplateTaxonomy.getConceptByID("t-op_BankingActivitiesDimension"),
					opTemplateTaxonomy.getConceptByID("d-ba_TotalBankingActivitiesSubjectBIA"));
			@SuppressWarnings("unused")
			MultipleDimensionType mdt2 = new MultipleDimensionType(opTemplateTaxonomy.getConceptByID("t-op_BankingActivitiesDimension"),
					opTemplateTaxonomy.getConceptByID("d-ba_TotalBankingActivitiesSubjectSTAAlternative"));
			MultipleDimensionType mdt4 = new MultipleDimensionType(opTemplateTaxonomy.getConceptByID("t-op_BusinessLinesDimension"),
					opTemplateTaxonomy.getConceptByID("d-bl_CorporateFinance"));
			mdt4.addPredecessorDimensionDomain(new SingleDimensionType(opTemplateTaxonomy.getConceptByID("t-op_BankingActivitiesDimension"), opTemplateTaxonomy
	...
	public void corepInstance() {
		...
		try {
			InstanceFactory.get().createInstance(new File(getClass().getResource(PATH2 + "FW1_instance.xml").toURI()));
			InstanceValidator validtator = new InstanceValidator(new File(getClass().getResource(PATH2 + "FW1_instance.xml").toURI()));
	...
	public void xbrlFact() {
		try {
			...
			assertTrue(instance.getFact(year3Fact.getConcept(), ctx.getDimensionalInformation(GeneralConstants.DIM_SCENARIO), GeneralConstants.DIM_SCENARIO) != null);
			assertTrue(instance.getFact(capitalRequirementsFact.getConcept(), myMDT, GeneralConstants.DIM_SCENARIO) != null);
			assertFalse(instance.getFact(capitalRequirementsFact.getConcept(), ctx.getDimensionalInformation(GeneralConstants.DIM_SCENARIO),
			...
			try {
				...
				_ctx.setIdentifierScheme("http://www.c-ebs.org/eu/fr/esrs/corep");
